 ...
 * 
 * Vercel Serverless Function for Gemini AI Content Generation
 * Migrated from server.ts Express endpoint for Vercel deployment
 */
...
function getGenAI(): GoogleGenAI {
  if (!ai) {
    ...
    ai = new GoogleGenAI({
      apiKey: apiKey,
      httpOptions: {
        headers: { "User-Agent": "vercel-practicum-automator" },
      },
    });
  }
  return ai;
    ...
    try {
      ...
      if (is503Error(e)) {
        if (attempt < delays.length) {
          ...
          console.log(
            `[API] 503/Transient error on ${modelName}. Retrying in ${delay}ms... (Retry ${attempt + 1}/${delays.length})`
          );
...
`;
    ...
    if (!response) {
      ...
      const isQuota =
        ...
        String(errorMsg).includes("Quota exceeded") ||
        String(errorMsg).includes("RESOURCE_EXHAUSTED");
      const statusText = isQuota ? "Quota exceeded" : "AI service temporarily unavailable";
      res.status!(200).json!({ success: false, error: statusText });
      return;
    }
